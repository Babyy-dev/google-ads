  ...
  try {
    ...
    switch (reportType) {
      case 'click_performance':
        data = await googleAds.getClickPerformanceData(dateRange || 'LAST_7_DAYS');
        break;
      case 'search_terms':
      ...
      default:
        return NextResponse.json(
          { error: 'Invalid report type. Use: click_performance, search_terms, or budget_data' },
          { status: 400 }
        );
    }
  ...
  return NextResponse.json({
    ...
    endpoints: {
      ...
      parameters: {
        ...
        reportType: 'string (required) - click_performance | search_terms | budget_data',
